...
float Compute3dRidgedNoise(float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, float ridgeGain, bool renormalize, unsigned int seed)
{
	const float OCTAVE_OFFSET = 0.636764989593174f; // same irrational offset as Perlin

	float totalNoise = 0.f;
	float totalAmplitude = 0.f;

	float amplitude = 0.5f; // starting amplitude
	float weight = 1.0f;    // previous octave influence
	float invScale = (1.f / scale);
	...
	for (unsigned int octaveNum = 0; octaveNum < numOctaves; ++octaveNum)
	{
		// --- Step 1: Sample base Perlin (no renormalization) ---
		float n = Compute3dPerlinNoise(currentPos.x, currentPos.y, currentPos.z,
			1.f, 1, 1.f, 1.f, false, seed);

		// --- Step 2: Convert to ridged shape ---
		n = 1.f - fabsf(n); // invert absolute noise (ridges become bright)
		n *= n;              // sharpen ridges

		// --- Step 3: Weight and accumulate ---
		n *= weight;
		totalNoise += n * amplitude;
		totalAmplitude += amplitude;

		// --- Step 4: Prepare next octave ---
		weight = GetClampedZeroToOne(n * ridgeGain);  // gain controls how strongly ridges propagate
		amplitude *= octavePersistence;
		currentPos *= octaveScale;

		currentPos.x += OCTAVE_OFFSET;
		currentPos.y += OCTAVE_OFFSET;
		currentPos.z += OCTAVE_OFFSET;
		++seed;
	}

	const float invTotalAmplitude = 1.f / totalAmplitude;
	// --- Step 5: Optional renormalization ---
	if (renormalize && totalAmplitude > 0.f)
	{
		totalNoise *= invTotalAmplitude;
		totalNoise = GetClampedZeroToOne(totalNoise);          // ensure [0, 1]
		totalNoise = SmoothStep3(totalNoise);      // soft remap to push toward extremes
		totalNoise = (totalNoise * 2.0f) - 1.f;    // map back to [-1, 1]
	}
