...
RWStructuredBuffer<KelpInstance> OutInstances : register(u0);
...
StructuredBuffer<KelpInstance> instanceBuffers[MAX_NUM_BINDLESS_BUFFERS] : register(t0, space1);
...
float GetKelpRippleAmount(float modelHeight, uint seed, float rigidity, float time)
{
    float motionScale = (1.0f - saturate(rigidity));
    float normalizedHeight = saturate(modelHeight);

    float instancePhase = GetRandomValueInRange(seed + 17u, 0.0f, 6.2831853f);
    float rippleSpeed = GetRandomValueInRange(seed + 53u, 0.8f, 1.2f);
    float rippleFrequency = GetRandomValueInRange(seed + 91u, 5.0f, 7.0f);

    float ripplePhase = (time * rippleSpeed) - (normalizedHeight * rippleFrequency) + instancePhase;
    float ripple = sin(ripplePhase);

    float amplitudeWeight = (normalizedHeight * normalizedHeight);

    float rippleAmount = ripple;
    rippleAmount *= 10.0f;
    rippleAmount *= amplitudeWeight;
    rippleAmount *= motionScale;
    return rippleAmount;
}

float GetKelpRippleDerivative(float modelHeight, uint seed, float rigidity, float time)
{
    float motionScale = (1.0f - saturate(rigidity));
    float normalizedHeight = saturate(modelHeight);
    ...
    float amplitudeWeight = (normalizedHeight * normalizedHeight);
    float amplitudeWeightDerivative = (2.0f * normalizedHeight);

    float rippleValueScale = (10.0f * motionScale);

    float derivative =
        ((cos(ripplePhase) * (-rippleFrequency)) * amplitudeWeight) +
        (sin(ripplePhase) * amplitudeWeightDerivative);

    derivative *= rippleValueScale;
    return derivative;
}

float3 ApplyKelpWobble(float3 localPosition, float modelHeight, uint seed, float rigidity, float time)
{
    float rippleAmount = GetKelpRippleAmount(modelHeight, seed, rigidity, time);

    float3 result = localPosition;
    result.x += rippleAmount;
    result.y += (rippleAmount * 0.75f);
    return result;
}
...
v2p_t VertexMain(vs_input_t input, uint instanceId : SV_InstanceID)
{
    ...
    KelpInstance kelpInstance = instanceBuffers[vegBufferIndex][instanceId];

    float randomizedHeightScale = GetRandomValueInRange(kelpInstance.seed + 0u, c_vegetationHeightMin, c_vegetationHeightMax);
    float randomizedWidthScale = GetRandomValueInRange(kelpInstance.seed + 1u, c_vegetationWidthMin, c_vegetationWidthMax);

    float4x4 worldTransform = kelpInstance.worldTransform;

    float3 localPosition = input.modelPosition;
    float modelHeight = input.modelPosition.z;

    localPosition.x *= randomizedWidthScale;
    localPosition.y *= randomizedWidthScale;
    localPosition.z *= randomizedHeightScale;

    localPosition = ApplyKelpWobble(localPosition, modelHeight, kelpInstance.seed, c_rigidity, c_time);

    float4 worldPosition = mul(worldTransform, float4(localPosition, 1.0f));
    ...
    float rippleDerivative = GetKelpRippleDerivative(modelHeight, kelpInstance.seed, c_rigidity, c_time);

    float3 bentLocalTangent = float3(randomizedWidthScale, 0.0f, 0.0f);
    float3 bentLocalBitangent = float3(0.0f, randomizedWidthScale, 0.0f);
    float3 bentLocalSpine = float3(rippleDerivative, (rippleDerivative * 0.75f), randomizedHeightScale);

    bentLocalTangent = normalize(bentLocalTangent);
    bentLocalBitangent = normalize(bentLocalBitangent);
    bentLocalSpine = normalize(bentLocalSpine);

    float3 bentLocalNormal = normalize(cross(bentLocalSpine, bentLocalTangent));
    bentLocalBitangent = normalize(cross(bentLocalNormal, bentLocalTangent));

    float3 worldTangent = normalize(mul(worldTransform, float4(bentLocalTangent, 0.0f)).xyz);
    float3 worldBitangent = normalize(mul(worldTransform, float4(bentLocalBitangent, 0.0f)).xyz);
    float3 worldNormal = normalize(mul(worldTransform, float4(bentLocalNormal, 0.0f)).xyz);
    ...
    v2p.modelTangent = bentLocalTangent;
    v2p.modelBitangent = bentLocalBitangent;
    v2p.modelNormal = bentLocalNormal;
...
float4 PixelMain(v2p_t input, bool isFrontFace : SV_IsFrontFace) : SV_Target0
{
    ...
    float4 finalColor = float4(litCoralColor, terrainDepthFraction);
